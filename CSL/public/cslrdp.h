...
public:
	...
	void ClearActions() throw();
	void AddAction(uint32_t uActionID, const std::shared_ptr<IRdParserAction>& sp);
	void SetAcceptedAction(const std::shared_ptr<IRdParserAction>& sp) throw();
...
private:
	//<action-id, action>
	std::map<uint32_t, std::shared_ptr<IRdParserAction>>  m_map;
	std::shared_ptr<IRdParserAction>  m_spAccepted;
